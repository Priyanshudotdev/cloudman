...
export const sshDeploy = async (
  ...
  const run = async (
    ...
    if (result.stdout) onLog({ step, message: result.stdout });
    if (result.stderr) onLog({ step, message: result.stderr });
    return {
  ...
  try {
    ...
    const serveInstall = await run(
      'installing_node',
      'sudo npm install -g serve',
    );
    if (serveInstall.code !== 0) {
      throw new Error(`Install serve failed: ${serveInstall.stderr}`);
    }

    onLog({
      step: 'cloning',
      message: `Cloning ${vars.githubRepoUrl}...`,
    });
    ...
    if (vars.projectType === 'react-vite') {
      ...
      if (buildResult.stdout) onLog({ step: 'building', message: buildResult.stdout });
      if (buildResult.stderr) onLog({ step: 'building', message: buildResult.stderr });
    ...
    onLog({ step: 'error', message: `Deploy failed: ${msg}`, success: false });
    ssh.dispose();
